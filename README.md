...
- **Auto-updating results** - the app reads live scores from ESPN's public scoreboard on its own.
...
| `.github/workflows/update-results.yml` | Historical reference only; superseded 2026-06-28 by client-side ESPN reads in `app.js` |
...
5. **Teams** - manual override: the admin can mark teams out / crown the champion by hand (useful if ESPN lags).
...
3. **Results auto-update on their own.** The app reads live scores client-side from ESPN's free public scoreboard, so there is nothing to enable and no secrets needed.
...
Live results are built client-side in `app.js` from a single source: ESPN's public scoreboard. In one fetch, `buildSchedule(espn)`:

- numbers the events 1..104 by ascending `event.id` (a self-consistent order),
- reads each match's stage from `season.slug` and the group letter from `data.json`,
- reads the teams, score, and status from the competitors,
- turns knockout slot labels like "Round of 32 3 Winner" into the bracket refs (`W<num>`/`L<num>`),
- derives each team's alive/out status and the champion.

This is **semi-live**: latency depends on how quickly ESPN updates after a match. If it ever lags, the admin can set results by hand in the **Teams** tab.

**openfootball was dropped on 2026-06-28.** The app previously took fixture/bracket structure from [openfootball/worldcup.json](https://github.com/openfootball/worldcup.json) and overlaid live scores from ESPN, but openfootball's knockout bracket wiring lagged the group results by hours (leaving Round-of-32 slots as placeholders like "1I" or "3A/B/C/D/F" after the group stage), which made the qualification logic wrongly mark qualified teams as OUT. ESPN resolves the knockout teams immediately, so it is now the single data source. Note that ESPN orders knockout matches differently from openfootball, so the bracket numbering is now ESPN's own. `scripts/update_results.py` (openfootball-based) is no longer run or used.
